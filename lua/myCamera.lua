module(...,package.seeall)

require"pm"
require"utils"
require"common"
require"mySocket"
require "misc"
require "net"

local DEFAULT_WIDTH,DEFAULT_HEIGHT = 640,480
local gc0310_ddr_big =
{
	zbar_scan = 0,
	i2c_addr = 0x21,
	sensor_width = 640,
	sensor_height = 480,
	id_reg = 0xf1,
	id_value = 0x10,
	spi_mode = disp.CAMERA_SPI_MODE_LINE2,
	spi_speed = disp.CAMERA_SPEED_DDR,
	spi_yuv_out = disp.CAMERA_SPI_OUT_Y1_V0_Y0_U0,

	init_cmd =
	{

	  ,

	},
}
function takePhotoAndSend()
	local tm = misc.getClock()
	log.info("testNtp.printTime", string.format("%04d/%02d/%02d,%02d:%02d:%02d", tm.year, tm.month, tm.day, tm.hour, tm.min, tm.sec))
	-- 16:50 分上传
	if tm.hour == 16 and tm.min == 50 then 
		-- net.switchFly(false)
		--唤醒系统
		pm.wake("takePhoto")
		ret = disp.cameraopen_ext(gc0310_ddr_big)
		log.info("testScanCode cameraopen_ext ret ", ret)
		disp.camerapreviewzoom(0xff)
		disp.camerapreview(0,0,0,0,DEFAULT_WIDTH,DEFAULT_HEIGHT)
		disp.cameracapture(DEFAULT_WIDTH,DEFAULT_HEIGHT)
		disp.camerasavephoto("/testCamera.jpg")
		log.info("-----testCamera.takePhotoAndSend fileSize",io.fileSize("/testCamera.jpg"))
		--关闭摄像头预览
		disp.camerapreviewclose()
		--关闭摄像头
		disp.cameraclose()
		pm.sleep("takePhoto")
		--允许系统休眠
		mySocket.sendFile(io.fileSize("/testCamera.jpg"))
		-- net.switchFly(true)
	end
   
end

sys.timerLoopStart(takePhotoAndSend,30*1000)
